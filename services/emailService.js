...
const getTransporter = () => {
  ...
  if (!user || !pass || user.includes('your-email')) {
    console.error('Email Service: GMAIL_USER or GMAIL_PASS not set in environment variables');
    return null;
  }

  console.log(`Email Service: Using account ${user}`);
...
export const sendReportEmail = async (patientEmail, patientName, testType, reportUrl) => {
  ...
  if (!transporter) {
    console.error('Email Service: Credentials not configured in environment variables');
  ...
  const mailOptions = {
    ...
    html: `
      <div style="font-family: sans-serif; padding: 20px; color: #333;">
        ...
        <p style="font-size: 0.8em; color: #999;">(c) 2026 BioCore Pathology Labs. Professional Diagnostic Services.</p>
      </div>
    `
  };
